...
import 'package:runlog/domain/run_tracker.dart';
import 'package:runlog/domain/step_provider.dart';
import 'helpers.dart';
...
void main() {
  ...
  group('state machine', () {
    test('starts idle and ignores samples until started', () {
      ...
      final outcome = tracker.addSample(
        sampleAt(northMeters: 0, eastMeters: 0, at: t0),
      );
  ...
  group('filter chain', () {
    test(
      'F1 rejects imprecise fixes, but the first fix gets a looser gate',
      () {
        tracker.start();
    ...
    test('F4 rejects a GPS teleport without disturbing the run', () {
      ...
      expect(
        tracker.addSample(outlier),
        SampleOutcome.rejectedImplausibleSpeed,
      );
    ...
    test('F4b stops a rejection cascade from letting a jump through', () {
      ...
      expect(
        outcomes.take(2),
        everyElement(SampleOutcome.rejectedImplausibleSpeed),
      );
  ...
  group('live metrics', () {
    ...
    test(
      'speed decays toward zero when the runner stops, and does not freeze',
      () {
        tracker.start();
        feed(straightLine(start: t0, count: 61, metersPerFix: 3));
  ...
  group('step-counter fallback', () {
    ...
    test(
      'losing location hands over to steps immediately, not after a timeout',
      () {
        tracker.start();
        feed(straightLine(start: t0, count: 30, metersPerFix: 3));
